      ...
      <Box sx={{ bgcolor: "primary.main" }}>
        <PhotoProvider>
          <ImageList
            ...
            variant="standard"
            cols={(width < 500 && 2) || (width < 800 && 3) || (width < 1100 && 4) || (width < 1400 && 5) || 6}
            gap={8}
          >
            {images.map((item) => (
              <PhotoView src={item.src} key={item.src}>
                <ImageListItem className="pointer" key={item.src}>
                  <img
                    src={`${resizeBase}?src=${encodeURIComponent(item.src)}&height=300&fit=cover`}
                    srcSet={`${resizeBase}?src=${encodeURIComponent(item.src)}&height=300&fit=cover 2x`}
                    alt=""
                    loading="lazy"
                  />
                </ImageListItem>
